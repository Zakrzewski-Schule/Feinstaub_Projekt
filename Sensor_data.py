...
class Sensor_Feinstaub_Data(Sensor_Data):
  ...
  def assign(self, csv):
    ...
    self.durP1 = float(csv['durP1'] or 0)
    self.ratioP1 = float(csv['ratioP1'] or 0)
    self.P2 = float(csv['P2'])
    self.durP2 = float(csv['durP2'] or 0)
    self.ratioP2 = float(csv['ratioP2'] or 0)
    return self
...
class Sensor_Temperature_Data_List(Sensor_Data_List):
  def __init__(self, iterable):
    self.min = 0.0
    self.max = 0.0
    self.avg = 0.0
...
class Sensor_Feinstaub_Data_List(Sensor_Data_List):
  def __init__(self, iterable):
    self.P1min = 0.0
    self.P1max = 0.0
    self.P1avg = 0.0
  ...
  def assign_calculated(self):
    ...
    self.P2min = min(P2s)
    self.P2max = max(P2s)
    self.P2avg = (sum(P2s) / len(P2s))
    self.P2diff = (max(P2s) - min(P2s))
    self.P2min_index = P2s.index(self.P2min)
    self.P2max_index = P2s.index(self.P2max)